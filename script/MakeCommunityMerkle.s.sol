...
/**
 * @title MakeMerkle_Community
 * @notice Reads the community input JSON generated by GenerateInput_Community.s.sol,
 *         builds the Merkle tree, and writes proofs + root to output_community.json.
 *
 * Usage:
 *   1. forge script script/GenerateCommunityInput.s.sol  → generates input_community.json
 *   2. forge script script/MakeCommunityMerkle.s.sol     → generates output_community.json
 *   3. Use the root from output_community.json when deploying the community MerkleClaim contract.
 *
 * Output → /script/target/output_community.json
 */
contract MakeCommunityMerkle is Script, ScriptHelper {
    using stdJson for string;

    Merkle private m = new Merkle();

    string private inputPath  = "/script/target/input_community.json";
    string private outputPath = "/script/target/output_community.json";
    ...
    function run() public {
        console.log("Generating community Merkle tree from %s", inputPath);
        ...
        console.log("DONE: Community Merkle tree written to %s", outputPath);
        console.log("Use the root value when deploying the community MerkleClaim.sol");
    }
}
